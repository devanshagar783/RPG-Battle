    ...
    loop@while (running){
        ...
        loop1@ for(player in players){
            ...
            if (index == 0){
                ...
                println("You attacked ${enemies[enemy].name} for ${dmg} points of damage")
                ...
                if (spell.type == "White"){
                    ...
                    println("\n${spell.name} deals ${magic_dmg} points of damage to ${enemies[enemy].name}")

                    if (enemies[enemy].get_hp() == 0){
                        println(enemies[enemy].name + " is dead.")
                        var ask = enemies.removeAt(enemy)
                        if (enemies.size==0) {
                            print("YOU WON!!!")
                            running = false
        ...
        for (enemy in enemies){
            ...
            if (enemyChoice==0){
                ...
                println("${enemy.name} deals ${enemyDamage} points of damage to ${players[target].name}")

                if (players[target].get_hp()==0){
                    println(players[target].name + " has died.")
            ...
            if (enemyChoice==1){
                ...
                if (spell.type=="White"){
                    enemy.heal(magicDmg)
                    println("${spell.name} heals ${enemy.name} for ${magicDmg} HP.")
                }
                else if (spell.type == "Black"){
                    var target = (0 until players.size).random()
                    players[target].take_dmg(magicDmg)
                    println("${enemy.name} deals ${magicDmg} points of damage to ${players[target].name}")
